...
对于用户、组、文档等核心资源，优先采用状态位和时间戳做软删除，以支持审计和恢复。
...
用于存放可选扩展资料，可根据实现合并入 `users`。V1 可先不强制拆表。
...
- 普通索引：`document_id`
- 普通索引：`document_version_id`
- 组合索引：`document_id, asset_kind`
...
- 组合索引：`resource_type, resource_id`
- 组合索引：`subject_type, subject_id`
- 组合索引：`subject_type, subject_key`
...
这套模型能够支撑首版“上传、分组、预览、下载、密码访问、组内访问、指定用户访问”的关键业务闭环。
